# Work Pack Index (human-readable)

**Machine authority:** `config/work_pack_registry.yaml`  
**Truth order:** `docs/TRUTH_ORDER.md`

---

## Closed — Block A @ `1037950`

| WP | Receipt |
|----|---------|
| WP-GROK-WONDER-PLACE-001 | `receipts/cursor-execution-wp-grok-block-a-20260616.md` |
| WP-GROK-CULTURAL-VICTORY-001 | same |
| WP-GROK-POLICY-BRANCH-001 | same |

Closure doc: `receipts/BLOCK-A-CLOSURE-20260616.md`

---

## Closed — Block B

| WP | Receipt |
|----|---------|
| WP-GROK-COMPETITION-DEPTH-001 | `receipts/cursor-execution-wp-grok-competition-depth-001-20260616.md` |
| WP-GROK-PLAYER-AGENT-001 | `receipts/cursor-execution-wp-grok-player-agent-001-20260616.md` |

Closure doc: `receipts/BLOCK-B-CLOSURE-20260616.md`

---

## Closed — Block C @ `a78e185`

| WP | Receipt |
|----|---------|
| WP-GROK-CULTURAL-EPILOGUE-001 (D-H1) | `receipts/cursor-execution-wp-grok-block-c-20260616.md` |
| WP-GROK-SOAK-001 | same |
| WP-GROK-AI-DIPLOMACY-001 | same |
| WP-GROK-DOMINATION-PATH-001 | same |
| WP-TOOLING-AWARENESS-001 | same |
| WP-GROK-DOC-SYNC-001 | same |
| WP-GROK-REFRACTOR-SIM-002 | same → `docs/SIM_MILESTONE_SYNC_DECISION_V1.md` |

Closure doc: `receipts/BLOCK-C-CLOSURE-20260616.md`

---

## Closed — Block D

| WP | Receipt |
|----|---------|
| WP-GROK-JWT-IDENTITY-001 | `receipts/cursor-execution-wp-grok-block-d-20260616.md` |

Closure doc: `receipts/BLOCK-D-CLOSURE-20260616.md`

---

## Grok handoff (all blocks)

**Primary bootstrap:** `receipts/HANDOFF-GROK-EXECUTION-PACK-20260616.md`

---

## Retired / superseded

| ID | Reason |
|----|--------|
| WP-GROK-AGENT-VS-AGENT-003 | Monolith rejected — use Block B WPs |
| WP-GROK-BLOCK-A-RESUBMIT-* | Block A closed — use BLOCK-A-CLOSURE |
| WP-BIAS-ROOT-CAUSE-ANALYSIS-041 | → `docs/AGENT_CLAIMS_POLICY.md` |
| WP-ROOT-CAUSE-DEEP-DIVE-001 | → `docs/TRUTH_ORDER.md` |
| WP-HINDSIGHT-DEBT-ANALYSIS-001 | → `docs/DEBT_REGISTER_V1.md` |
| WP-HISTORY-ORPHAN-REVIEW-042 | Report-only orphan inventory |
| WP-TOOLING-AWARENESS-PROPOSAL-001 | → WP-TOOLING-AWARENESS-001 (Block C) |

---

## Governance docs (read first)

| Doc | Purpose |
|-----|---------|
| `docs/TRUTH_ORDER.md` | Authority stack |
| `docs/WORK_PACK_LIFECYCLE.md` | State machine |
| `docs/AGENT_CLAIMS_POLICY.md` | Banned claims |
| `docs/DEBT_REGISTER_V1.md` | Engineering debt |
| `AGENTS.md` | Agent bootstrap |
