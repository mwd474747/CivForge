...
- **Strict Separation** (see SEPARATION.md): CivForge = governance layer only. Gravity-mosaic = independent target (only via deploy.sh). dawsos-auth-prototype = sister identity root (only via the sibling repo and `tools/dawsos_auth_client.py`). Three roots — do not conflate. Use handoff artifacts for cross-workspace.
...
- **Write-It-Down**: receipts/ + Cursor execution receipts. Grok swarm handoff: `prompts/grok_swarm_handoff_seed.md`. Lanes: `docs/EXECUTION_LANE_V2.md`.
...
See: AGENTS.md (full roles/bootstrap), ORCHESTRATION_PATTERNS.md (loops), SEPARATION.md (roots), docs/patterns/borrowable-governance-patterns.md (reuse), HANDOFF_CONTEXT.md (other agents), tools/dawsos_auth_client.py + deploy.sh (bridges).
