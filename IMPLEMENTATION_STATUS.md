...
**Verification**:
- `bash tools/verify-truth-anchor.sh` (read-only) then `bash tools/validate-game.sh --read-only`
- `bash tools/check-agent-shell-hygiene.sh` (optional `--kill` for stale Cursor wrapper shells)
